...
public class ApplicationHelper {
	...
	/*
	 * For Terracotta. ThreadLocal is JVM specific so we can't use method
	 * getCurrentApplication(). See tc-config.xml <on-load> elements.
	 */
	public static ApplicationContext getApplicationContext(Application app) {
		ServletContext sc = ((WebApplicationContext) app.getContext())
	...
	public static VaadinApplicationObjectSupport getVaadinApplicationObjectSupport() {
		String names[] = getApplicationContext().getBeanNamesForType(
				VaadinApplicationObjectSupport.class);
		if (names != null && names.length == 1) {
			return (VaadinApplicationObjectSupport) getApplicationContext()
					.getBean(names[0]);
		}
		return null;
	}

	public static Resource getFileResource(String location)
			throws URISyntaxException {
		URL url = ApplicationHelper.class.getResource(
				location);
		if (url == null) {
			return null;
		}
		return new FileResource(new File(url.toURI()), getCurrentApplication());
	}
}

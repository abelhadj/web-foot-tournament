...
public class TournamentService extends ServiceImpl<Tournament> implements ITournamentService {
	...
	private void createSubCupNode(CupNode cupNode, int depth,
		...
		if ((depth == 0 && cupStructure.get(1) > 0)
				|| (depth == 1 && cupStructure.get(1) <= 0 && cupStructure
						.get(2) > 0)) {
	...
	public Tournament autoAssignUnassignedTeams(Tournament tournament) {
		...
		if (tournament instanceof Championship) {
			...
			Iterator<GamingTeam> it = championship.getLeague()
					.getGamingTeamsStats().keySet().iterator();
			while (it.hasNext()) {
	...
	private void assignTeamFromList(GamingTeam gamingTeam, List<Team> teams) {
		if (!teams.contains(gamingTeam.getTeam())) {
			Team random = teams.get(new Random(System.currentTimeMillis()).nextInt(teams.size()));
